...
SYSTEM_PROMPT = (
    ...
    "Identify and describe the differences between the two images in detail. "
    "Provide a concise summary of the key differences, including any added or removed elements, changes in colors, shapes, or objects. "
    "Your response should be structured as follows:\n"
    "1. A brief explanation of the overall differences.\n"
    "2. A list of bullet points highlighting specific changes.\n"
    "3. An optional confidence score (0 to 1) indicating your certainty about the differences identified.\n"
    "4. Lists of added and removed elements if applicable.\n"
    "Ensure your analysis is clear, objective, and free from personal opinions."
)
...
class VLMService:
    ...
    def __init__(self):
        ...
        self.default_model_id = os.environ.get("MODEL_ID", "OpenGVLab/InternVL3-1B-hf") # Example model, replace with actual VLM model
    ...
    def ensure_local_model(self, model_id: Optional[str] = None):
        ...
        print(f"[VLMService] Loading local model {model_id} on {device} (this may take a while)...")
    ...
    def compare_images(self, imgA: Image.Image, imgB: Image.Image, model_id: Optional[str] = None, use_hf_api: bool = False)-> DifferenceResponse:
        ...
        if use_hf_api:
            ...
            if not hf_token:
                raise RuntimeError("HF_TOKEN environment variable is required for Hugging Face Inference API mode.")
            text_out = self._call_local_model(imgA, imgB, prompt)

        bullets, added, removed = self._postprocess_text_bullets(text_out)
